...
def _fire_event(callback: EventCallback, event_type: str, data: dict) -> None:
    """
    Planifie l'émission d'un événement SSE sur la boucle asyncio en cours
    depuis un contexte synchrone (outil LangGraph @tool).
    """
    try:
        loop = asyncio.get_running_loop()
        loop.create_task(callback(event_type, data))
    except RuntimeError:
        pass  # Pas de boucle active — on ignore silencieusement
...
def _make_chef_tools(
    ...
    async def delegate_to_mettreur(ouvrage_id: int, task_description: str) -> str:
        ...
        report = last.content if isinstance(last, AIMessage) else str(last)
...
def _build_mettreur_graph(
    ...
    builder.add_conditional_edges("mettreur", tools_condition)
...
def _build_chef_graph(
    ...
    builder.add_conditional_edges("chef", tools_condition)
